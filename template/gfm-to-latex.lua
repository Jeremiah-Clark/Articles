...
--   3. Page breaks before level-1 headings (always) and level-2 headings (optional)
...
-- \clearpage before H2 — optional, controlled by the h2-page-break metadata
--   field in master.yaml (default: false). Useful for documents where each
--   subsection should start on its own page.
...
-- ---------------------------------------------------------------------------
-- With the GFM reader, Pandoc reads dash proportions from the
...
-- ---------------------------------------------------------------------------
-- If a paragraph's last character is a colon, it's almost certainly
-- introducing the content below it. Inject \nopagebreak between them
-- so LaTeX won't split them across pages.
